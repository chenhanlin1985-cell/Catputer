...
def main():
    ...
    parser.add_argument("--source-dir", default=r"d:\clawputer\sdcard\pet\ime")
    ...
    with serial.Serial(args.port, 115200, timeout=10, write_timeout=10) as port:
        ...
        send_cmd(port, {"cmd": "sd_mkdir", "path": "/pet/ime"})
        for file_path in sorted(source_dir.glob("*.txt")):
            push_file(port, file_path, f"/pet/ime/{file_path.name}")
            print(f"Pushed {file_path.name}")

    print("SD IME sync complete.")
