...
Catputer 现在由三部分组成，但当前开发主线已经收束到 Waveshare 触屏端：

- **Cardputer 端侧**：键盘形态的随身宠物设备，支持离线养成、中文输入、聊天、BLE 键盘等能力；当前进入维护归档状态，短期不再主动新增宠物表现功能。
- **Waveshare 触屏端侧**：触摸屏形态的随身宠物设备，当前重点是沉浸式主页、触摸交互、天气/时间、SD 资源与 SD OTA。
- **PC 猫舍**：Godot 桌面端，用于管理多只宠物、查看状态/记忆，并把宠物送往不同端侧或从端侧接回。

当前核心思路是：触屏端负责“陪伴感”和即时反馈，PC 端负责“管理、AI 世界事件和更大屏的信息展示”，SD 卡负责可替换资源与离线更新。后续不再把 Cardputer 当作宠物表现主舞台。
...
- 当前为维护归档端：保留可编译和已有能力，短期不作为新增功能主线。
- 支持离线基础宠物循环。
...
- 支持触摸宠物触发关注回合：一段时间未关注后再触摸，会显示重逢短句和最近小事；联网可用时会把这段上下文交给 AI 生成更自然的陪伴回复。
...
- 优先继续把触屏端做成“轻交互、强陪伴”的主端侧，而不是堆复杂输入。
- PC 猫舍继续承担多宠物管理、记忆查看、配置入口、端侧选择和 AI 世界事件生成。
- AI 的重点不是泛泛聊天，而是生成宠物在“小世界里生活”的短事件、重逢反馈和场景化气泡。
- Cardputer 进入维护归档状态，除非明确需要，不再启动新功能开发。
