...
static void _on_apb_change(void *arg, apb_change_ev_t ev_type, uint32_t old_apb, uint32_t new_apb)
{
  ...
  {
    uint32_t freq = old_apb / ((_spi->dev->clock.clkdiv_pre + 1) * (_spi->dev->clock.clkcnt_n + 1));
#if defined(ESP_ARDUINO_VERSION_MAJOR) && (ESP_ARDUINO_VERSION_MAJOR >= 3)
    _spi->dev->clock.val = spiFrequencyToClockDiv(_spi, freq);
#else
    _spi->dev->clock.val = spiFrequencyToClockDiv(freq);
#endif
    SPI_MUTEX_UNLOCK();
  }
}
